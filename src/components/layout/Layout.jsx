...
export default function Layout({ title = "Shiv Holidays", children }) {
    ...
    return (
        <>
            {/* MOBILE OVERLAY */}
            <div
                className={`menu-overlay fixed inset-0 bg-black/50 z-40 transition-all duration-300 ${
            ...
            {/* NAVBAR */}
            <nav
                className={`fixed w-full top-0 z-50 backdrop-blur-md border-b border-white/20 shadow-sm transition-all duration-300 ${
                    navbarScrolled ? "bg-white/95" : "bg-white/30"
                }`}
            >
                <div className="container mx-auto px-6 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Link to="/" className="flex items-center z-50">
                            <img
                                src="/images/SHIV HOLIDAYS LOGO.png"
                    ...
                    {/* MOBILE MENU */}
                    <div
                        className={`md:hidden bg-white rounded-lg shadow-xl mt-4 overflow-hidden transition-all duration-300 ${
...
function MobileLink({ to, label, close, small = false }) {
    return (
        <Link
            to={to}
            onClick={close}
            className={`block px-4 transition duration-300 rounded-lg ${
                small
                    ? "py-2 text-sm text-gray-600 hover:text-brand"
                    : "py-3 text-gray-800 hover:bg-brand hover:text-white font-medium"
            }`}
        >
            {label}
        </Link>
...
function HamburgerIcon({ active }) {
    return (
        <div className="flex flex-col justify-center">
            <span
                className={`block w-6 h-[3px] bg-gray-800 transition-all duration-300 mb-1 rounded ${
            ...
            />
            <span
                className={`block w-6 h-[3px] bg-gray-800 transition-all duration-300 rounded ${
                    active ? "-rotate-45 -translate-y-2" : ""
                }`}
            />
        </div>
...
function FooterLink({ to, label }) {
    return (
        <Link
            to={to}
            className="text-gray-300 hover:text-brand flex items-center group"
        >
